...
import static it.finanze.sanita.fse2.ms.gtw.rulesmanager.repository.entity.structures.base.StructuresETY.FIELD_ID;
import static it.finanze.sanita.fse2.ms.gtw.rulesmanager.repository.entity.structures.base.StructuresETY.FIELD_LAST_UPDATE;
...
public class StructureRepo implements IStructureRepo {
    ...
    private Document getCurrentDocByDate(MongoCollection<Document> src) throws EdsDbException {
        ...
        try {
            ...
            doc = src.find()
                .sort(new Document().append(FIELD_LAST_UPDATE, -1))
                .first();
    ...
    public void createEmptyDocOnParent(String name, String... fields) throws EdsDbException {
        ...
        // Append fields
        document = document.append(FIELD_LAST_UPDATE, new Date());
        // Iterate
    ...
    public void updateDueToEmptyOnParent(String name) throws EdsDbException {
        ...
        if(!parent.containsKey(FIELD_LAST_UPDATE)) {
            throw new EdsDbException("No field exists on parent document matching: " + FIELD_LAST_UPDATE);
        }
        // Update document
        try {
            // Execute
            src.updateOne(
                eq(FIELD_ID, parent.getObjectId(FIELD_ID)),
                currentDate(FIELD_LAST_UPDATE)
            );
        }catch (MongoException e) {
